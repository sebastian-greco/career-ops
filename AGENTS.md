...
Three ownership classes — full list in `DATA_CONTRACT.md`:

- **User Layer (NEVER auto-updated; personalization goes HERE):** `cv.md`, `config/profile.yml`, `modes/_profile.md`, `modes/_custom.md`, `article-digest.md`, `portals.yml`, `resumes/*.json`, `data/*`, `reports/*`, `output/*`, `interview-prep/*`
- **Local Fork Extensions (manual merge only; preserve during upstream integrations):** `web-dashboard/*`, `modes/apply-full.md`, `modes/json-cv.md`, `modes/scan-jobgether.md`, `modes/scan-wttj.md`, the local RxResume/external-tracker scripts, and `.agents/skills/humanizer-zh/*`
- **System Layer (auto-updatable; DON'T put user data here):** upstream modes and scripts not listed as local extensions, `modes/_shared.md`, `AGENTS.md`, `CLAUDE.md`, `CODEX.md`, `OPENCODE.md`, `KIMI.md`, `GEMINI.md`, `dashboard/*`, `templates/*`, `batch/*`
...
- `resumes/*.json` (candidate-authored RxResume bases; facts must remain consistent with the sources above)
...
| `resumes/*.json` | Candidate-authored RxResume bases used by the review-first `json-cv` path |
| `sync-rxresume.mjs` / `validate-resume-json.mjs` / `verify-rxresume-pdf.mjs` | Local RxResume sync, JSON validation, and one-page PDF verification |
| `sync-application-tracker.mjs` | External NextStage tracker synchronization used by application modes |
...
| Wants a review-first tailored RxResume JSON | `json-cv` |
| Explicitly authorizes the full reviewed application flow for one report | `apply-full` |
...
- `cv.md` in project root remains the canonical factual CV
- `article-digest.md` has detailed proof points (optional)
- `resumes/*.json` are the candidate-authored presentation bases for the local RxResume workflow; `json-cv` copies and minimally tailors one of these bases rather than rebuilding a resume from scratch
...
- **`apply` is read-only in this fork.** It may inspect the live JD/form, draft answers, and synchronize selective tracker context, but it never fills fields, uploads documents, clicks through the form, or submits.
- **`apply-full` is the only submission authority.** An explicit `/career-ops apply-full <report-id>` invocation authorizes filling, uploading the independently reviewed RxResume PDF, and submitting that one application after every gate in `modes/apply-full.md` passes.
- **Fail closed on material uncertainty.** Missing factual answers, identity values, legal attestations, work authorization, compensation, authentication, CAPTCHA, or reviewer escalation pause the flow; preserve state and resume only after the user supplies or completes what is required.
- **No implicit submission from discovery.** Scans, evaluations, pipeline processing, schedules, `apply`, and `json-cv` never authorize submission.
