...
This document defines which files belong to the **system** (auto-updatable), the **user** (never touched by updates), or this fork's **local extensions** (preserved and merged manually).
...
| `resumes/*.json` | Your candidate-authored RxResume base documents and presentation choices |
...
## Local Fork Extensions (manual merge only)

These files are code and workflow extensions specific to this fork. They are not personal-data files, but upstream updates must preserve them and merge changes manually rather than replacing them.

| File | Purpose |
|------|---------|
| `web-dashboard/*` | The existing custom local dashboard, kept alongside the upstream `web/` experience |
| `modes/apply-full.md` | Explicit, gated authority for one reviewed application submission |
| `modes/json-cv.md` | Review-first RxResume JSON tailoring workflow |
| `modes/scan-jobgether.md` / `modes/scan-wttj.md` | Logged-in browser discovery workflows |
| `sync-rxresume.mjs` / `validate-resume-json.mjs` / `verify-rxresume-pdf.mjs` | Local RxResume sync, validation, and PDF verification |
| `sync-application-tracker.mjs` / `evaluate-job.mjs` | External tracker synchronization and local evaluation integration |
| `.agents/skills/humanizer-zh/*` | Local Chinese humanizer skill |
...
| Upstream `*.mjs` entries in `update-system.mjs` | Utility scripts shipped by upstream (excluding the local extensions above) |
...
**If a file is a Local Fork Extension, preserve it and merge upstream changes into it manually.**
